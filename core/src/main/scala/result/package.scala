/** =Use Case=
  ...
  *
  *   - [[Result.err err]] transforms [[Result]]`[E, T]` into `Option[E]`,
  ...
  *
  *   - [[Result.map map]] transforms [[Result]]`[E, T]` into [[Result]]`[E, U]`
  *     by applying the provided function to the contained value of [[Ok]] and
  *     leaving [[Err]] values unchanged
  *
  * This method transforms the contained value of the [[result.Err Err]]
  * variant:
  *
  *   - [[Result.mapErr mapErr]] transforms [[Result]]`[E, T]` into
  *     [[Result]]`[F, T]` by applying the provided function to the contained
  *     value of [[Err]] and leaving [[Ok]] values unchanged
  *
  * These methods transform a [[Result]]`[E, T]` into a value of a possibly
  * different type `U`:
  *
  *   - [[Result.mapOr mapOr]] applies the provided function to the contained
  *     value of [[Ok]], or returns the provided default value if the [[Result]]
  *     is [[Err]]
  *   - [[Result.mapOrElse mapOrElse]] applies the provided function to the
  *     contained value of [[Ok]], or applies the provided default fallback
  ...
  *
  * These methods extract the contained value in a [[Result]]`[E, T]` when it is
  ...
  *
  * These methods extract the contained value in a [[Result]]`[E, T]` when it is
  * the [[Err]] variant. If the [[Result]] is [[Ok]]:
  *
  ...
  * ==Boolean operators==
  *
  * These methods treat the [[Result]] as a boolean value, where [[Ok]] acts
  * like `true` and [[Err]] acts like `false`. There are two categories of these
  * methods: ones that take a [[Result]] as input, and ones that take a function
  * as input (to be lazily evaluated).
  *
  * The [[Result.and and]] and [[Result.or or]] methods take another [[Result]]
  * as input, and produce a [[Result]] as output. The [[Result.and and]] method
  * can produce a [[Result]]`[E, U]` value having a different inner type `U`
  * than [[Result]]`[E, T]`. The [[Result.or or]] method can produce a
  * [[Result]]`[F, T]` value having a different error type `F` than
  * [[Result]]`[E, T]`.
  *
  ...
  * a new value. The [[Result.andThen andThen]] method can produce a
  * [[Result]]`[E, U]` value having a different inner type `U` than
  * [[Result]]`[E, T]`. The [[Result.orElse orElse]] method can produce a
  * [[Result]]`[F, T]` value having a different error type `F` than
  * [[Result]]`[E, T]`.
  *
  * NOTE: [[Result.flatMap flatMap]] is equivalent to [[Result.andThen andThen]]
  * and it is also provided to consistency with typical Scala patterns.
  *
  ...
  *
  * @note
  *   This documentation is a derivative of the
